...
@test "src/setup_ssh/ssh_status.sh, function safely_check_ssh_service_is_enabled: functions if ssh service available and running." {
  ...
  run safely_check_ssh_service_is_enabled

  # Verify result is as expected.
  assert_output "FOUND"
}

@test "src/setup_ssh/ssh_status.sh, function safely_check_ssh_service_is_enabled: functions if ssh service is not available." {
...
@test "src/setup_ssh/ssh_status.sh, function safely_check_ssh_service_is_enabled: functions if ssh service available but disabled." {
  # Ensure openssh-server is installed.
  apt_remove "openssh-server" 1
  ensure_apt_pkg "openssh-server" 1
